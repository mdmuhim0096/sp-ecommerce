...
const PurchaseSuccess = () => {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      toast.error("sessionId not found in this URL");
    }
  ...
  useEffect(() => {
    setTimeout(() => { setConfetti(true); }, 12300);
  }, []);

  return (
    <div className="w-full min-h-screen max-h-max flex flex-col">
      <Navbar />

      <div className="flex-1 flex flex-col items-center pt-16 gap-8 py-10 min-h-screen max-h-max">
        <style>{`
          :root {
            --primary: #275EFE;
          ...
          .order .truck .front { overflow: hidden; position: absolute; border-radius: 2px 9px 9px 2px; width: 26px; height: 41px; left: 60px; }
        ...
        <button
          className={`order ${animate ? "animate" : ""} sm:scale-[1.6] md:scale-[2] top-[40%]`}
        >
        ...
        {next && (
          <motion.div
            ...
            className="w-full sm:w-7/12 md:w-4/12 rounded-lg py-6 "
          >
            {isConfetti && <Confetti
            ...
            <h1
              ...
              className="text-center my-4 font-bold text-2xl text-white"
            >
              Congratulations
            </h1>
